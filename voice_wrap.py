...
async def stt_loop(ui: StatusUI, stt: STTAdapter, submit_cb: Callable[[str], None], interrupt_cb: Callable[[], None],
    ...
    if ui.live:
        ui.live.update(ui.render()); ui.live.refresh()

    try:
        # Fake provider: do not require a real microphone for dry tests.
        if isinstance(stt, FakeStreaming):
            async def dummy_pcm():
                while True:
                    # yield empty chunks periodically to keep any consumers unblocked
                    yield b""
                    await asyncio.sleep(0.05)

            async for ev in stt.stream(dummy_pcm(), metadata={"sr": sr}):
    ...
    # Real mic path (AssemblyAI or other providers)
    if not isinstance(stt, FakeStreaming):
        try:
                ...
                async for ev in stt.stream(mic.chunks(), metadata={"sr": sr}):
                    ...
                    if ev.is_final and seg:
                        ...
                        if len(sgl.split()) <= 3 and fuzzy_match_any(sgl, SEND_WORDS):
                            to_send = ui.buffer.strip()
                            ui.note = "Submitting…"
                            if ui.live: ui.live.update(ui.render()); ui.live.refresh()
                            if to_send:
                                submit_cb(to_send + "\n")
                            ui.clear()
                            ui.note = "Submitted."
...
async def pass_through_loop(child: PTYChild, ui: StatusUI):
    ...
    try:
        with Live(
            ...
            ui.live = live
            # Auto-start listening in fake mode (or if VOICE_AUTOSTART is true)
            auto_env = os.getenv("VOICE_AUTOSTART")
            autostart = False
            if auto_env is not None:
                autostart = auto_env.lower() in ("1", "true", "yes", "on")
            else:
                autostart = isinstance(stt, FakeStreaming)

            if autostart and not listening_task:
                ui.listening = True
                ui.note = "Starting mic…"
                live.update(ui.render()); live.refresh()
            ...
            while True:
                ...
                if child.master_fd in r:
                    try:
                        ...
                        if not data:
                            break
                        # Print child output via Rich so Live can keep panel anchored
                        try:
                            text = data.decode("utf-8", "replace")
                        except Exception:
                            text = data.decode("latin-1", "replace")
                        live.console.print(text, end="")
                        live.refresh()
                ...
                # No periodic UI refresh; only on actual changes.
...
async def main():
    ...
    if "--" in sys.argv:
        ...
        # Fallback: some launchers (e.g., `poetry run`) may consume '--'.
        # If extra args exist, treat them as the child command.
        if len(sys.argv) > 1:
            argv = sys.argv[1:]
            console.print("[grey62]Note: '--' not detected; treating remaining args as child command.[/grey62]")
        else:
    ...
    if not argv:
        console.print("[red]No child command provided[/red]")
        sys.exit(1)
